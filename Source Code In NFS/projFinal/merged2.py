    ...
    try:
        ...
        for i in range(1, size):
            if i != rank:
                ...
                comm.send((IMAGE[start:end], start, end, operation), dest=i)  # Send the task
...
def redistribute_workload_batch(operation, start, end):
    ...
    try:
        ...
        for i in range(1, size):
            if i != rank:
                comm.send((True,), dest=i)  # Notify the node to prepare for receiving the task
                comm.send(( IMAGELIST[start:end], start, end, operation), dest=i)  # Send the task
...
if rank == 0 and name=='Master':   # Master node
...
elif (name!='Master'and name!='Slave3'): # Worker nodes
